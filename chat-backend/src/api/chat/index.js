...
function firstSendMessage(user, ws) {
    MESSAGE.queryMessageByUserId(user.id, MESSAGE.STATE.SERVER_RECEIVING).then(
        (messages) => {
            ...
            redis.set('msuid' + user.id, messageString).then((res) => {
                console.log(res);
            });
        },
    );
...
function sendNewMessage(user, ws) {
    redis.get('msuid' + user.id).then((messages) => {
        messages = messages ? JSON.parse(messages) : [];
        messages = messages.filter(
            (item) =>
                item.state == MESSAGE.STATE.REVOKE ||
                item.state == MESSAGE.STATE.SERVER_RECEIVING,
        );
        ws.send(
            JSON.stringify({
...
function receiveMessage(userId, messages) {
    if (messages.length > 0) {
        ...
        redis.get('msuid' + userId).then((redisMessages) => {
...
function revokeMessage(message, ws) {
    MESSAGE.queryById(message.id).then((message) => {
        ...
        if (
            ...
            // 找到该消息的所有接收者
            MESSAGE.setMessageStateByMessageId(
                message.id,
                MESSAGE.STATE.REVOKE,
            ).then((states) => {
                if (states) {
                    states.map((item) => {
                        setState(item);
                ...
                JSON.stringify({
                    data: {
                        ...
                        key: {
                            ...
                            type:
                                message.sendType >
                                MESSAGE.SEND_TYPE.FRIEND_MESSAGE
                                    ? RELATION.TYPE.GROUP
                                    : message.sendType,
                        },
                    },
...
function setGroupMessage(message, ws) {
    RELATION.queryInGroupRelation(message.sendId, message.receiveId).then(
        (relation) => {
            if (relation) {
                MESSAGE.addNewMessageState(message.sendId, message.id);
                RELATION.queryGroupRelation(message.receiveId).then(
                    (relations) => {
