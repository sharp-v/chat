...
const { USER_DATA_CACHE_TIME, WS_TYPE,
  verToken, setUserWsAlive, setUserWsDead,
  clearUser, saveState,
  updateMessages, setMessageTimeOut } = require('../common')
      ...
      if (states != null) {
        ...
        states.map(item => {
          ...
          if (item.state == MESSAGE.STATE.SERVER_RECEIVING) {
            redis.get('mid' + item.messageId)
              .then(msg => {
                if (msg) {
                  messages.push(JSON.parse(item))
                } else {
                  MESSAGE.queryById(item.messageId)
                    .then(msg => {
                      messages.push(msg)
                    })
                }
