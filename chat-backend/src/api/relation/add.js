...
function addUserRelation(relation, res) {
  ...
  RELATION.addNewUserRelation(relation)
    .then(relation => {
      if (relation) {
        switch (relation[0].type + '' + relation[1].type) {
          ...
          case RELATION.TYPE.STRANGER + '' + RELATION.TYPE.FRIEND:
            ...
            RELATION.updateRelation(relation[0])
              .then(newRelation => {
                res.send({
...
function addRelationByReminderLevel(relation, res) {
  switch (relation.reminderLevel) {
    ...
    case RELATION.REMINDER_LEVEL.RECEIVE_AND_NOT_NOTIFY:
      addSystemNotifyMessage(relation.contactId, relation, code.system.notify.newFriend)
        .then(flag => {
...
function addGroupRelation(relation, res) {
  GROUP.queryGroupOwn(relation.contactId)
    .then(group => {
      addSystemNotifyMessage(group.ownId, relation, code.system.notify.newGroupMember)
        .then(flag => {
