...
confirmAdd.post(
    ...
    (req, res) => {
        try {
            verToken(req.headers['token'], req).then((verfiyResult) => {
...
function agreeBeUserFriend(relation, res) {
    RELATION.updateRelation(relation).then((newRelation) => {
        if (newRelation) {
            addSystemNotifyMessage(
                newRelation.contactId,
                newRelation,
                code.system.notify.newFriendNotify,
            );
            addSystemNotifyMessage(
                newRelation.userId,
                newRelation,
...
function agreeBeGroupMember(relation, res) {
    GROUP.queryGroupOwn(relation.contactId).then((group) => {
        if (group.ownId == user.id) {
            RELATION.queryGroupRelation(user.id, group.id).then((relations) => {
