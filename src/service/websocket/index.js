...
async function distributeMessage(messages) {
    ...
    await Promise.all(
        messages.map((message) => {
            ...
            if (
                store.state.message.indexMap.get(message.messageId) ==
                    undefined ||
                ...
                switch (main.sendType) {
                    ...
                    case MESSAGE.SEND_TYPE.FRIEND_MESSAGE:
                        ...
                        if (message.main.sendId == message.userId) {
                            key = {
                                id: message.main.receiveId,
                                type: USER_TYPE,
                            };
                            message = judgeRevoke(
                                message,
                                MESSAGE.MY_REVOKE_REPLACE,
                            );
                            commitMessage(
                                'myMessage',
                                JSON.stringify(key),
                            ...
                            message = judgeRevoke(
                                message,
                                MESSAGE.RELATION_REVOKE_REPLACE,
                            );
                            commitMessage(
                                'newMessage',
                                JSON.stringify(key),
                    ...
                    case MESSAGE.SEND_TYPE.GROUP_ALL_MESSAGE:
                        ...
                        message.senderKey = JSON.stringify({
                            id: main.sendId,
                            type: USER_TYPE,
                        });
                        if (message.main.sendId == message.userId) {
...
const wsConnection = {
    ...
    initConfig: function (config) {
        const _this = this;
        const unconfigurableSet = new Set([
            'BASEURL',
            'maxReconnectTimes',
            'heartBeatPackage',
        ]);
    ...
    wsMsg: function (msg) {
        ...
        if (data.code) {
            ...
            switch (true) {
                case wscode == code.chat.open.sendMessage.value ||
                    wscode == code.chat.message.heartBeat.value:
                    // 提交到relation.*.message上
                    distributeMessage(data.data).then((arr) => {
                        arr.length > 0 &&
                            wsConnection.wsSend(WS_TYPE.RECEVICE_MESSAGE, arr);
                    });
                    break;
    ...
    reconnect: function () {
        if (
            this.connectFlag &&
            this.reconnectTimes - this.maxReconnectTimes < 0
        ) {
            this.reconnectTimes++;
            this.initWebSocket();
        } else {
